...
import {
  ...
  CheckCircle, XCircle, Table, BookOpen, Handshake,
} from "lucide-react"
...
function FinanceContent() {
  ...
  useEffect(() => {
    document.title = activeTab === "pricing" ? "Pricing — LUMARY Studio" : "Finance — LUMARY Studio"
  }, [activeTab])
  ...
  function switchTab(tab: string) {
    ...
    if (tab === "finance") params.delete("tab")
    else params.set("tab", tab)
    router.push(`/finance${params.toString() ? `?${params.toString()}` : ""}`, { scroll: false })
  }

  return (
    <div className="space-y-6 stagger-children">
      {/* Tabs */}
      <div className="flex gap-1 bg-white/60 border border-outline-variant/20 rounded-2xl p-1 w-fit">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            ...
            className={clsx(
              ...
              activeTab === tab.key
                ...
                : "text-on-surface-variant/70 hover:text-on-surface hover:bg-black/[0.04]"
            )}
          >
            {tab.label}
          </button>
