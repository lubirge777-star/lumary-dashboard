...
import { Lightbulb, Plus, X, Loader2, AlertCircle, RefreshCw, Save } from "lucide-react"
...
export default function IdeasPage() {
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ideas"] })
      toast("success", "Status Updated", "Idea status has been changed")
    },
    onError: (err: Error) => toast("error", "Failed", err.message),
  })
      ...
      {isLoading ? (
        ...
        <div className="glass-card p-card-padding flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-2xl bg-surface-variant/50 border border-outline-variant/30 flex items-center justify-center mb-5">
            <Lightbulb className="w-7 h-7 text-on-surface-variant/80" />
          </div>
          <h3 className="text-base font-semibold text-on-surface mb-1.5">No ideas yet</h3>
          <p className="text-sm text-on-surface-variant/70 max-w-sm mb-6">Start validating your first idea by clicking "New Idea"</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter">
          {ideas.map((idea) => (
